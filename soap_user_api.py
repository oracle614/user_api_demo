...
class UserService(ServiceBase):
    ...
    def getAll(self):
        global data
        for user in data:
            yield str(user['id']) + ', ' + user['name'] + ', ' + user['passwd']
    ...
    def checkUser(self, name, passwd):
        ...
        if user:
            return '验证成功' if md5(passwd) == user[0].get('passwd') else '用户名或密码错误'
        else:
            return '用户不存在'
    ...
    def addUser(self, name, passwd): 
        ...
        if user:
            ...
            data.append({"id": id, "name": name, "passwd": md5(passwd)})
            ...
            user = list(filter(lambda user: user.get('name') == name, data))
            if user:
                return '添加成功' if md5(passwd) == user[0].get('passwd') else '添加失败，密码错误'
            else:
                return '添加失败，用户不存在'
    ...
    def modifyUser(self, name, passwd): 
        ...
        if not user:
            ...
            id = int(data[-1].get('id')) + 1
            user[0].update({"id": id, "name": name, "passwd": md5(passwd)})
            with open(DB_FILE, 'w') as f:
                json.dump(data, f)
            with open(DB_FILE, encoding='utf-8') as f:
                data = json.load(f)
            user = list(filter(lambda user: user.get('name') == name, data))
            if user:
                return '修改成功' if md5(passwd) == user[0].get('passwd') else '修改失败，密码错误'
            else:
                return '修改失败，用户不存在'
